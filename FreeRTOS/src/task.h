...
{
	/*
	 * Defines the info of periodic tasks
	 */
...
//	   unsigned char task_id;
	   ...
	   int tick_period;
//	   int time_deadline;
//	   unsigned char dependency_task_id_list[7];
	   void (*task_function)(void*);  //pokazivac na metodu
	   uint16_t stack_depth;
	   void* task_param;  //parametar koji prosledjujemo metodi
	} task_info;
}
#endif
...
{
	/*
	 * Defines the info of aperiodic tasks
	 */
	typedef struct Task_info
	{
	   unsigned char task_id;
...
#if ( configUSE_EDF_PERIODIC == 1)
{
	/*
	* Defines the info of periodic tasks
	*/
	typedef struct Periodic_task_info
	{
		unsigned char task_id;
		int time_period;
		int time_length;
...
//	BaseType_t xAperiodicTaskCreate(TaskFunction_t pxTaskCode, const char * const pcName, const uint16_t usStackDepth,
//				void * const pvParameters, unsigned char task_id, TickType_t time_start, TickType_t time_legnth,
//				TickType_t time_deadline, unsigned char* dependency_task_id_list, UBaseType_t priority);

	void xAperiodicTaskCreate(task_info* new_task_info);
#endif
